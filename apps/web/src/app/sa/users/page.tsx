...
export default function SAUsersPage() {
  ...
  useEffect(() => {
    if (!token) return;
    apiAuthFetch('/sa/users', token).then(setUsers).catch(console.error).finally(() => setLoading(false));
  }, [token]);
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              {form.role === 'PARISH_ADMIN' && (
                <input className="w-full border rounded px-3 py-2 text-sm" placeholder="Parish ID (UUID)" value={form.parishId} onChange={(e) => setForm({ ...form, parishId: e.target.value })} />
              )}
            </div>
